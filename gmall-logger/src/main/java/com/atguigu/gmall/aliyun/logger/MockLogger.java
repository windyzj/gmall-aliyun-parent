package com.atguigu.gmall.aliyun.logger;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.atguigu.gmall.aliyun.logger.bean.*;
...
public class MockLogger {
    ...
    static int loop_len;
    static int mid_length;
    static int uid_length;
    static int itemid_len;
...
//    循环遍历次数
          loop_len = args.length > 1 ? Integer.parseInt(args[1]) : 10 * 100;
//    mid的长度
          mid_length = args.length > 2 ? Integer.parseInt(args[2]) : 3;
//    uid的长度
          uid_length = args.length > 3 ? Integer.parseInt(args[3]) : 5;
//    商品id的长度
          itemid_len = args.length > 4 ? Integer.parseInt(args[4]) : 6;
        ...
        for (int i = 0; i < loop_len; i++) {
            ...
            switch (flag) {
                ...
                case (1):
                    ...
                    // 商品详情页
                    if (rand.nextBoolean()) {
                        eventsArray.add(generatedDetail());
                    }

                    // 商品列表页
                    if (rand.nextBoolean()) {
                        eventsArray.add(generateList());
                    }
...
//          控制台打印
            log.info(millis + "|" + json.toJSONString());
        }
    }
...
//      纬度
     ...
     * @return
     */
    static JSONObject generateDisplay(   ) {
...
//      商品id
        appDisplay.setItemid(genItemId());
...
//      分类
     ...
     * @return
     */
    static JSONObject generatedDetail( ) {

        AppItemDetail appItemDetail = new AppItemDetail();

//      页面入口来源
        int flag = 1 + rand.nextInt(3);
        appItemDetail.setEntry(flag  );

//      动作
        appItemDetail.setAction(  (rand.nextInt(4) + 1));

//      商品id
        appItemDetail.setItemid( genItemId());

//      商品来源类型
        flag = 1 + rand.nextInt(3);
        appItemDetail.setShowtype(flag  );
...
//      页面停留时长
        ...
        appItemDetail.setStaytime (flag  );

//      加载时长
        flag = rand.nextInt(10) * rand.nextInt(7);
        appItemDetail.setLoading_time(flag  );



//      分类
        flag = 1 + rand.nextInt(100);
        appItemDetail.setCategory(  flag);

        JSONObject eventJson = (JSONObject) JSON.toJSON(appItemDetail);

        return packEventJson("item_detail", eventJson);
    }
     ...
     * @return
     */
    static JSONObject generateList() {
...
//      失败码
    ...
    static int getRandomDigitsInt(int len) {

        return rand.nextInt( (int) Math.pow(10,len) );

    }

    static  int genItemId(){
        return getRandomDigitsInt(itemid_len);
    }

    static  int genUId(){
        return getRandomDigitsInt(uid_length);
    }
    ...
    static public JSONObject generateCart() {
        ...
        AppItemCart appItemCart = new AppItemCart();
        appItemCart.setItemid(genItemId());
    ...
    static public JSONObject generateFavorites() {
        ...
        favorites.setItemid(genItemId());
        favorites.setUserid(genUId());
